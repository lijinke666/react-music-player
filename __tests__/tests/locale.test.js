...
describe('Locale test', () => {
  ...
  it('should render default locale with en_US', () => {
    ...
    expect(wrapper.find('.music-player-controller-setting').text()).toContain('Open')
    ...
    expect(wrapper.find('.audio-lists-panel .title').text()).toContain('Playlists')
    expect(getTitle(wrapper, '.audio-lists-panel .delete-btn')).toEqual('Delete audio lists')
    expect(getTitle(wrapper, '.audio-lists-panel .close-btn')).toEqual('Close')
    expect(getTitle(wrapper, '.audio-lists-panel .audio-item', 0)).toEqual('Click to play')
    expect(getTitle(wrapper, '.audio-lists-panel .player-delete', 0)).toEqual('Click to delete audioName')
  ...
  it('should render locale with zh_CN', () => {
    ...
    expect(wrapper.find('.music-player-controller-setting').text()).toContain('打开')
    expect(wrapper.find(FaHeadphonesIcon)).toHaveLength(1)
    ...
    // 打开播放列表
    wrapper.find('.audio-lists-btn').simulate('click')
    expect(wrapper.find('.audio-lists-panel .title').text()).toContain('播放列表 / 1')
    expect(getTitle(wrapper, '.audio-lists-panel .delete-btn')).toEqual('清空播放列表')
    expect(getTitle(wrapper, '.audio-lists-panel .close-btn')).toEqual('关闭')
    expect(getTitle(wrapper, '.audio-lists-panel .audio-item', 0)).toEqual('点击播放')
    expect(getTitle(wrapper, '.audio-lists-panel .player-delete', 0)).toEqual('点击删除 audioName')
